...
sc.object    <- CreateSeuratObject(counts= singleCellDataGenes[ , 2: (n.cells+1)], project= "Single_cell_data", min.cells= 0, min.features= 0) # 900 genes in Macosko, genes expressed in fewer than 0.05% of cells are exluded
dim(sc.object@assays$RNA@counts)  # [15,208, 4,391]
sc.object[['RNA']]
...
mito.genes <- grep(pattern = "^mt-", x = rownames(x = sc.object@assays$RNA@data), value = TRUE)
percent.mito <- Matrix::colSums(sc.object@assays$RNA@counts[mito.genes, ])/Matrix::colSums(sc.object@assays$RNA@counts)
...
n.ref.genes.ident.1= colSums(sc.object@assays$RNA@counts[which(rownames(sc.object@assays$RNA@counts) %in% reference.rgc.markers.3), ] > 0 )# [picked.cells]
n.ref.genes.ident.2= colSums(sc.object@assays$RNA@counts[which(rownames(sc.object@assays$RNA@counts) %in% reference.rgc.markers.3), ] > 0 )# [picked.cells]
cells.selected= which( names(x = sc.object@assays$RNA@data)  %in% names(x = sc.object@assays$RNA@data) )        # which cells are selected for downstream analysis
n.ref.genes.ident.1.df= data.frame(n.ref.genes.ident.1)
names(n.ref.genes.ident.1.df)= "num.ref.genes"
sc.object <- AddMetaData(object= sc.object, metadata= n.ref.genes.ident.1.df,  col.name= "num.ref.genes")         # adding number of reference genes to each cell
...
ave.expressed.genes.2= log ( x= rowMeans( x= exp(sc.object@assays$RNA@data) -1) + 1 )
ave.expressed.genes.2["Thy1"]

# back to original scale from log [cell expr/sum(cell expr)*10.000 +1]
expressed.genes.3= log (    ( ( exp(as.matrix(sc.object@assays$RNA@data)) -1)/10000.0 )*matrix( rep(colSums(exp(sc.object@assays$RNA@data) -1), each= nrow(sc.object@assays$RNA@data)), nrow=nrow(sc.object@assays$RNA@data) )  + 1 )
